 ...
 *
 * Elements inside the laptop rig ([data-laptop-rig]) and its catalogue
 * miniature ([data-laptop-mini]) are excluded from all global hooks so
 * they don't receive reveal/parallax animations that would fight the
 * flight controller.
 *
...
export default function ScrollExperience({
  ...
  useGSAP(
    () => {
      ...
      /**
       * Global hooks must never touch the rig or the screen slider.
       * The screen slider is owned by the laptop flight controller.
       */
      const pick = (selector: string) =>
        (Array.from(stage.querySelectorAll(selector)) as HTMLElement[]).filter(
          (el) =>
            !el.closest("[data-laptop-rig]") &&
            !el.closest("[data-laptop-mini]"),
        );
